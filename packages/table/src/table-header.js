              ...
              >
                {
                  columns.map((column, cellIndex) => {
                    let sortableNode;
                    if (column.sortable) {
                      let ascendingNode = column.sortable === 'ZAndA'
                        ? (<i class="sort-caret-letter ascending" on-click={ ($event) => this.handleSortClick($event, column, 'ascending') }>A-Z</i>)
                        : (<i class="sort-caret ascending" on-click={ ($event) => this.handleSortClick($event, column, 'ascending') }></i>);

                      let descendingNode = column.sortable === 'ZAndA'
                        ? (<i class="sort-caret-letter descending" on-click={ ($event) => this.handleSortClick($event, column, 'descending') }>Z-A</i>)
                        : (<i class="sort-caret descending" on-click={ ($event) => this.handleSortClick($event, column, 'descending') }></i>);

                      sortableNode = (<span class="caret-wrapper" on-click={ ($event) => this.handleSortClick($event, column) }>{ascendingNode}{descendingNode}</span>);
                    }
                    return (
                      <th
